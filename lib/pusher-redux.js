...
// create redux action
var pusherAction = function pusherAction(options) {
  var result = {
...
var successfullyConnected = function successfullyConnected(res) {
  isConnected = true;
  config.store.dispatch(pusherAction({ actionType: CONNECTED, eventName: res.event, data: res.data }));
  runPending();
};

var disconnected = function successfullyConnected() {
  isConnected = true;
